...
function findRendering(keyword: string) {
  ...
  if (idx !== -1) {
    ...
    console.log(js.substring(Math.max(0, idx - 100), Math.min(js.length, idx + 1000)));
  } else {
    console.log(`--- No match for "${keyword}" ---`);
  }
}

findRendering('id:"demo-tour-360"');
findRendering('id:"demo-model-rotate"');
findRendering('id:"demo-camera"');
